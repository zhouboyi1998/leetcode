...
* 使用一个数组 `arr` 记录 `"c"`、`"cr"`、`"cro"`、`"croa"`、`"croak"` 出现的次数
* 遍历字符串，根据字符对数组 `arr` 对应下标的值进行 `++` 操作
...
* 此时需要将表示字符串 `"croa"` 数量的 `arr[3]` 加一
* 同时还需要将表示字符串 `"cro"` 数量的 `arr[2]` 减一
* 因为使用了一个字符串 `"cro"` 去拼接成 `"croa"` 了

<br/>

* 因为一只青蛙可以多次蛙鸣
* 而 `arr[4]` 此时表示的是字符串 `"croak"` 的数量，也就是**完整蛙鸣的次数**
* 所以当字符 `'c'` 出现时，如果此时 `arr[4]` 的值大于 `0`
* 说明是其中一只青蛙在结束了一次蛙鸣后，开始进行第二次蛙鸣，`arr[4]` 需要减一
* 这样 `arr[4]` 最终的含义将会变成**青蛙的数量**，而不再是**完整蛙鸣的次数**
...
class Solution {
    public int minNumberOfFrogs(String croakOfFrogs) {
        ...
        return arr[0] == 0 && arr[1] == 0 && arr[2] == 0 && arr[3] == 0 ? arr[4] : -1;
    }
}
```
